...
type recordRow struct {
	...
	MagCal           vec3
	MagPred          vec3
	MagEMA           bool // 1 if the display-path EMA was on for this row
	TempC            float64
	GPS              gpsFix
	TrackMagDeg      float64
	N0Ut             float64
	DeclDeg          float64
	InclDeg          float64
	AlignHeadingDeg  float64 // mag heading baked into the active alignment, NaN if unaligned
